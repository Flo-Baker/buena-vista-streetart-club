...
router.post("/login", (req, res, next) => {
    ...
    if( !email || !password ) {
        res.render('auth/login.hbs', { errorMessage: "You need to fill out the whole form."});
        return;
    }
    ...
    .then((user) => {
        ...
        res.redirect('/profile')
            ...
            res.render('auth/login.hbs', { errorMessage: "Wrong password, please try again." } || res.render('auth/signup.hbs', { errorMessage: "Username not found, please create your profile. Thanks." }))           
        } 
    })
    .catch((err) => {
        next(err)
